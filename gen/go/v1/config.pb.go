...
type Auth struct {
	...
	Disabled      bool        `protobuf:"varint,1,opt,name=disabled,proto3" json:"disabled,omitempty"`                      // DEPRECATED: use auth_driver="disabled". When true it takes precedence over auth_driver.
	Users         []*User     `protobuf:"bytes,2,rep,name=users,proto3" json:"users,omitempty"`                             // users to allow access to the UI. only valid when auth_driver is "local".
	AuthDriver    string      `protobuf:"bytes,3,opt,name=auth_driver,json=authDriver,proto3" json:"auth_driver,omitempty"` // "disabled", "local" (default when empty), or "oidc".
	Oidc          *OidcConfig `protobuf:"bytes,4,opt,name=oidc,proto3" json:"oidc,omitempty"`                               // OpenID Connect settings. only valid when auth_driver is "oidc".
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Auth) GetAuthDriver() string {
	if x != nil {
		return x.AuthDriver
	}
	return ""
...
func (x *Auth) GetOidc() *OidcConfig {
	if x != nil {
		return x.Oidc
	}
	return nil
...
type OidcConfig struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	IssuerUrl      string                 `protobuf:"bytes,1,opt,name=issuer_url,json=issuerUrl,proto3" json:"issuer_url,omitempty"`                // OIDC issuer URL (used for .well-known discovery). required.
	ClientId       string                 `protobuf:"bytes,2,opt,name=client_id,json=clientId,proto3" json:"client_id,omitempty"`                   // OAuth2 client ID. required.
	ClientSecret   string                 `protobuf:"bytes,3,opt,name=client_secret,json=clientSecret,proto3" json:"client_secret,omitempty"`       // OAuth2 client secret. optional (empty for public clients).
	Scopes         []string               `protobuf:"bytes,4,rep,name=scopes,proto3" json:"scopes,omitempty"`                                       // OAuth2 scopes to request. defaults to openid, email, profile when empty.
	RedirectUrl    string                 `protobuf:"bytes,5,opt,name=redirect_url,json=redirectUrl,proto3" json:"redirect_url,omitempty"`          // OAuth2 redirect URL. optional; derived from the request host when empty.
	AllowedEmails  []string               `protobuf:"bytes,6,rep,name=allowed_emails,json=allowedEmails,proto3" json:"allowed_emails,omitempty"`    // if set, only these email addresses may log in. optional.
	AllowedDomains []string               `protobuf:"bytes,7,rep,name=allowed_domains,json=allowedDomains,proto3" json:"allowed_domains,omitempty"` // if set, only emails in these domains may log in. optional.
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
func (x *OidcConfig) GetAllowedEmails() []string {
	if x != nil {
...
func (x *OidcConfig) GetAllowedDomains() []string {
	if x != nil {
...
const file_v1_config_proto_rawDesc = "" +
	...
	"\x06action\"\x8b\x01\n" +
	"\x04Auth\x12\x1e\n" +
	"\bdisabled\x18\x01 \x01(\bB\x02\x18\x01R\bdisabled\x12\x1e\n" +
	"\x05users\x18\x02 \x03(\v2\b.v1.UserR\x05users\x12\x1f\n" +
	"\vauth_driver\x18\x03 \x01(\tR\n" +
	"authDriver\x12\"\n" +
	"\x04oidc\x18\x04 \x01(\v2\x0e.v1.OidcConfigR\x04oidc\"\xf8\x01\n" +
	"\n" +
	"OidcConfig\x12\x1d\n" +
	"\n" +
	...
	"\x0eallowed_emails\x18\x06 \x03(\tR\rallowedEmails\x12'\n" +
	"\x0fallowed_domains\x18\a \x03(\tR\x0eallowedDomains\"Q\n" +
	"\x04User\x12\x12\n" +
...
var file_v1_config_proto_goTypes = []any{
	...
	(*Hook)(nil),                               // 17: v1.Hook
	(*Auth)(nil),                               // 18: v1.Auth
	(*OidcConfig)(nil),                         // 19: v1.OidcConfig
	(*User)(nil),                               // 20: v1.User
...
var file_v1_config_proto_depIdxs = []int32{
	...
	24, // 18: v1.RetentionPolicy.policy_time_bucketed:type_name -> v1.RetentionPolicy.TimeBucketedCounts
	...
	25, // 26: v1.Hook.action_command:type_name -> v1.Hook.Command
	26, // 27: v1.Hook.action_webhook:type_name -> v1.Hook.Webhook
	27, // 28: v1.Hook.action_discord:type_name -> v1.Hook.Discord
	28, // 29: v1.Hook.action_gotify:type_name -> v1.Hook.Gotify
	29, // 30: v1.Hook.action_slack:type_name -> v1.Hook.Slack
	30, // 31: v1.Hook.action_shoutrrr:type_name -> v1.Hook.Shoutrrr
	31, // 32: v1.Hook.action_healthchecks:type_name -> v1.Hook.Healthchecks
	32, // 33: v1.Hook.action_telegram:type_name -> v1.Hook.Telegram
	20, // 34: v1.Auth.users:type_name -> v1.User
	19, // 35: v1.Auth.oidc:type_name -> v1.OidcConfig
